...
// template <class OP>
// void mapredNaive(
//     const uint32_t B  // desired CUDA block size ( <= 1024, multiple of 32)
//     ,
...
//         uint32_t num_blocks = (T + B - 1) / B;
//         redNaiveKernel2<OP>
//             <<<num_blocks, B>>>(d_out, offs_inp, offs_out, 2 * T, T);
//         tmp = offs_inp;
//         offs_inp = offs_out;
//         offs_out = tmp;
//     }
...
//             <<<num_blocks, B, shmem_size>>>(d_out, d_in, N, num_seq_chunks);
//     }
...
//     //
//     redAssocKernel<OP, CHUNK>
//         <<<num_blocks, B, shmem_size>>>(d_tmp, d_in, N, num_seq_chunks);
...
//         <<<num_blocks, B, shmem_size>>>(d_out, d_in, d_tmp, N, num_seq_chunks);
// }
