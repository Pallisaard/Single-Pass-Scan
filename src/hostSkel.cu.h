...
uint32_t closestMul32(uint32_t x) { return ((x + 31) / 32) * 32; }

void log2UB(uint32_t n, uint32_t* ub, uint32_t* lg) {
    uint32_t r = 0;
    uint32_t m = 1;
    if (n <= 0) {
        printf("Error: log2(0) undefined. Exiting!!!");
        exit(1);
    }
    while (m < n) {
        r = r + 1;
 ...
 */
template <int CHUNK>
uint32_t getNumBlocks(const uint32_t N, const uint32_t b_size, uint32_t* num_chunks) {
    ...
    *num_chunks = max(1, N / min_elms_all_thds);
    ...
    const uint32_t num_blocks = (num_thds + b_size - 1) / b_size;

    if (num_blocks > MAX_BLOCK) {
        printf(
            "Broken Assumption: number of blocks %d exceeds maximal block "
            "size: %d. Exiting!",
...
template <class OP>
void mapredNaive(
    const uint32_t b_size  // desired CUDA block size ( <= 1024, multiple of 32)
    ,
...

    {
        // first invocation
        uint32_t T = q / 2;
        uint32_t num_blocks = (T + b_size - 1) / b_size;
        redNaiveKernel1<OP><<<num_blocks, b_size>>>(d_out, d_in, N, T);
    }
    ...
    for (int k = 2; k <= K; k++) {
        ...
        uint32_t num_blocks = (T + b_size - 1) / b_size;
        redNaiveKernel2<OP>
            <<<num_blocks, b_size>>>(d_out, offs_inp, offs_out, 2 * T, T);
        tmp = offs_inp;
        offs_inp = offs_out;
        offs_out = tmp;
    }
...
void mapReduce(
    ...
    if (OP::commutative) {
        ...
        num_blocks = min(MAX_BLOCK, (T0 + b_size - 1) / b_size);
        const uint32_t T = num_blocks * b_size;
        const size_t shmem_size = b_size * sizeof(typename OP::RedElTp);
        redCommuKernel<OP><<<num_blocks, b_size, shmem_size>>>(d_out, d_in, N, T);
    } else {
        // if the operator is not commutative, things are more tricky because
        // the thread cannot traverse with a stride `T` as before (that
        // traversal assumes commutativity). It follows that each thread has to
        // (sequentially) process consecutive elements, but if done naively this
        // would result in uncoalesced access to global memory. To solve this we
        // need the threads in a block to cooperatively read from global memory
        // in coalesced fashion and put the result in shared memory, and then
        // read their elements from there.
        uint32_t num_seq_chunks;
        num_blocks = getNumBlocks<CHUNK>(N, b_size, &num_seq_chunks);
        const size_t shmem_size = b_size * max(sizeof(typename OP::InpElTp) * CHUNK,
                                          sizeof(typename OP::RedElTp));
        redAssocKernel<OP, CHUNK>
            <<<num_blocks, b_size, shmem_size>>>(d_out, d_in, N, num_seq_chunks);
    }
 ...
 * t_tmp is a temporary array (used to scan in-place across the per-block
 * results) Implementation consist of three phases:
...
void scanInc(
	...
	// printf("N: %d\n", N);
    uint32_t num_seq_chunks;
    const uint32_t num_blocks = getNumBlocks<CHUNK>(N, b_size, &num_seq_chunks);
    const size_t shmem_size = b_size * max_tp_size * CHUNK;

    //
    redAssocKernel<OP, CHUNK>
        <<<num_blocks, b_size, shmem_size>>>(d_tmp, d_in, N, num_seq_chunks);
    ...
    scan3rdKernel<OP, CHUNK>
        <<<num_blocks, b_size, shmem_size>>>(d_out, d_in, d_tmp, N, num_seq_chunks);
}
 ...
 * `d_out` is the result array (result of scan)
 * `d_tmp_vals`  is a temporary array (used to scan in-place across the
 * per-block results) `d_tmp_flags` is the temporary array of flags as above.
 */
template <class OP>  // element-type and associative operator properties
void sgmScanInc(
    const uint32_t b_size  // desired CUDA block size ( <= 1024, multiple of 32)
    ...
    uint32_t num_seq_chunks;
    const uint32_t num_blocks = getNumBlocks<CHUNK>(N, b_size, &num_seq_chunks);
    const size_t shmem_size = b_size * max(max_tp_size * CHUNK, tot_red_sz);

    redSgmScanKernel<OP, CHUNK><<<num_blocks, b_size, shmem_size>>>(
        d_tmp_flags, d_tmp_vals, d_flags, d_inp, N, num_seq_chunks);

    {
        const uint32_t block_size = closestMul32(num_blocks);
        const size_t shmem_size =
            block_size * (sizeof(typename OP::RedElTp) + sizeof(char));
        sgmScan1Block<OP><<<1, block_size, shmem_size>>>(
            d_tmp_vals, d_tmp_flags, num_blocks);
    }

    sgmScan3rdKernel<OP, CHUNK><<<num_blocks, b_size, shmem_size>>>(
