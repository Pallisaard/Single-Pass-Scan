...
#define OPTIMIZATION_2 1
#define OPTIMIZATION_3 1
...
#else
	// dummy implementation : sequential reduction by the first thread
	if (lane == 0) {
#pragma unroll
...
scanIncBlock(volatile typename OP::RedElTp* ptr, const unsigned int idx)
{
	...
	if (lane == (WARP - 1)) {
		...
		// This places all end results of each warp at the end of the first warp.
		// The race condition happens at block size 1024 because then we want the
		// First warp to write its last element in its first position while we want
		// the last warp to write its last element to the first warps position. The
		// fix is to instead write the value returned from scanIncWarp, which happens
		// to be the warp scan result.
		ptr[warpid] = res;
	}
	__syncthreads();
...
template <class OP>
__global__ void
redNaiveKernel1(typename OP::RedElTp* d_out, typename OP::InpElTp* d_in, const uint32_t N, const uint32_t T)
{
	uint32_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
template <class OP>
__global__ void
redNaiveKernel2(typename OP::RedElTp* d_out, const uint32_t offs_inp, const uint32_t offs_out, const uint32_t N, const uint32_t T)
{
	uint32_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
template <class OP>
__global__ void
redCommuKernel(typename OP::RedElTp* d_tmp, typename OP::InpElTp* d_in, uint32_t N, uint32_t T)
{
	extern __shared__ char sh_mem[];
...
__global__ void
redAssocKernel(typename OP::RedElTp* d_tmp, typename OP::InpElTp* d_in, uint32_t N, uint32_t num_seq_chunks)
{
	extern __shared__ char sh_mem[];
	...
	for (int seq = 0; seq < num_elems_per_block; seq += num_elems_per_iter) {
		...
		copyFromGlb2ShrMem<typename OP::InpElTp, CHUNK>(inp_block_offs + seq, N, OP::identInp(), d_in, shmem_inp);
...
template <class OP, int CHUNK>
__global__ void
scan3rdKernel(typename OP::RedElTp* d_out, typename OP::InpElTp* d_in, typename OP::RedElTp* d_tmp, uint32_t N, uint32_t num_seq_chunks)
{
	extern __shared__ char sh_mem[];
...
template <class OP>
__global__ void
sgmScan1Block(typename OP::RedElTp* d_vals, char* d_flag, uint32_t N)
{
	extern __shared__ char sh_mem[];
...
template <class OP, int CHUNK>
__global__ void
redSgmScanKernel(char* d_tmp_flag, typename OP::RedElTp* d_tmp_vals, char* d_flag, typename OP::InpElTp* d_in, uint32_t N, uint32_t num_seq_chunks)
{
	typedef ValFlg<typename OP::RedElTp> FVTup;
	extern __shared__ char sh_mem[];
	volatile typename OP::InpElTp* shmem_inp = (typename OP::InpElTp*)sh_mem;
	...
	for (int seq = 0; seq < num_elems_per_block; seq += num_elems_per_iter) {
		...
		copyFromGlb2ShrMem<typename OP::InpElTp, CHUNK>(inp_block_offs + seq, N, OP::identInp(), d_in, shmem_inp);
		copyFromGlb2ShrMem<char, CHUNK>(inp_block_offs + seq, N, 0, d_flag, shmem_flg);
...
template <class OP, int CHUNK>
__global__ void
sgmScan3rdKernel(typename OP::RedElTp* d_out, typename OP::InpElTp* d_inp, char* d_flg, typename OP::RedElTp* d_tmp_vals, char* d_tmp_flag, uint32_t N, uint32_t num_seq_chunks)
{
	...
	for (int seq = 0; seq < num_elems_per_block; seq += num_elems_per_iter) {
		...
		copyFromGlb2ShrMem<typename OP::InpElTp, CHUNK>(inp_block_offs + seq, N, OP::identInp(), d_inp, shmem_inp);
		copyFromGlb2ShrMem<char, CHUNK>(inp_block_offs + seq, N, 0, d_flg, shmem_flg);
